...
/// <summary>
/// <c>GET /api/establishments/opportunities/applications/{applicantId}</c>
/// (Laravel-compat) and
/// <c>GET /api/v1/establishments/{establishmentId}/browse/applications/{applicantId}</c>
/// (canonical) — detail of an application SUBMITTED BY the resolved
/// establishment. 404 when the application exists but belongs to a
/// different establishment.
/// </summary>
public sealed class GetEstablishmentApplicationEndpoint
    : EndpointWithoutRequest<OpportunityApplicationResponse>
{
    private readonly AppDbContext _db;
    private readonly ICurrentUser _currentUser;

    public GetEstablishmentApplicationEndpoint(AppDbContext db, ICurrentUser currentUser)
    {
        _db = db;
    ...
    {
        Get(
            "/api/establishments/opportunities/applications/{applicantId}",
            "/api/v1/establishments/{establishmentId}/browse/applications/{applicantId}");
        ...
        Summary(s =>
        {
            s.Summary = "Detail of an application submitted by the establishment.";
        });
    }
    ...
    public override async Task HandleAsync(CancellationToken ct)
    {
        ...
        if (application is null
            || application.ApplicantEstablishmentId != establishmentId.Value)
        {
            await Send.NotFoundAsync(ct);
            return;
        }
