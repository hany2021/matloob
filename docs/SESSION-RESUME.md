...
> **Latest session — backoffice HEAD `1c2df91`, frontend HEAD `30dabe4`, suite 512 → 529, both repos pushed & in sync.** A long multi-part session (details in §6, newest first):
> 1. **Closed the 3 remaining code-backlog items** (`e7d5cdd`): `event: null` hydration sweep (18 read paths), offer-list name search (`sender_name`/`applicant_name`), and the **Notifications FYI layer** (the LAST migration item — all 5 legacy DB notifications, no schema change).
> 2. **Drove accept / reject / evaluate live in-browser** end-to-end (IdM reachable again). Found + fixed a **frontend evaluation-endpoint bug** (`72d6fee` — individual evaluation hit the establishment endpoint), and **verified the notifications layer end-to-end live** (accept-driven `offer.is_active` + StatusSync-driven `event.started` both materialized via the dispatcher).
> 3. **Fixed the unevaluated-contracts page** "حدث خطأ ما!" (`30dabe4` — same individual-vs-establishment endpoint pattern as the eval bug) + a **TokenSync** improvement (stale-token-on-silent-renew) + made the OAuth scope env-overridable (the IdM client rejects `offline_access`, so refresh tokens need an IdM-side `AllowOfflineAccess` — verified live).
> 4. **Reviewed opportunity creation (all types)** and shipped 3 fixes: `working_hours_type` enum was `Fixed/Flexible/Shifts` instead of `full_time/part_time` so it was silently dropped (`1d4a6f1`); type-aware required validation (salary-for-vacancy, criteria-for-non-vacancy) + opportunity date-bounds (`eabc0d2`).
...
- **Backoffice** (`matloob-backoffice (.net + angular)`, remote = github.com/hany2021/matloob): `dev`, HEAD `1c2df91` — **pushed to `origin/dev` (in sync).** **Full .NET test suite: 529/529 green.** Session commits: `e7d5cdd` (3 backlog items) · `1d4a6f1` (working_hours_type enum) · `eabc0d2` (opp type-aware validation + date-bounds) + doc commits.
- **Frontend** (`matloob-frontend`, remote name **`github`** = github.com/EventsCenter/matloob-frontend — NOT `origin`; push with `git push github dev`): `dev`, HEAD `30dabe4` — pushed (in sync). Frontend changes this session: evaluation-endpoint fix (`72d6fee`) + unevaluated-list endpoint fix & TokenSync (`30dabe4`) — both individual-vs-establishment endpoint bugs; see live-QA notes below.
...
- ✅ **FIXED (`eabc0d2`) — type-aware required validation + date-bounds.** `EventOpportunitiesSupport.ValidateAsync` now enforces (multipart wizard + standalone paths): **`monthly_salary` required for vacancy** (legacy `RequiredForVacancy`), **≥1 `success_criterion` required for non-vacancy** (legacy `RequiredForOpportunity`), **`end_date > start_date`** as a clean 422 (was a domain `ArgumentException`/500), and **opp dates within the owning event's window** (legacy `ValidOpportunityStart/EndDate` — event dates threaded from step_two in the wizard or the persisted event standalone; skipped when unknown). +5 tests; suite 524→529. (Success-criteria *persistence* was already correctly gated to non-vacancy.)
...
Cross-referenced every Next.js call vs. implemented routes. **All code-backlog items are DONE, and accept/reject/evaluate is now live-verified** (see the Live-QA block below). No known outstanding migration work. Candidate follow-ups: (a) the two flagged opportunity-creation parity nits (description `min 100→10`, name `max 30→120`; JSON-path `success_criteria`) — low impact, frontend doesn't hit them; (b) a sweep for other **individual-page-reuses-Organizer-component** endpoint bugs (the eval-modal + unevaluated-list bugs shared that root cause); (c) enabling the OutboxDispatcher/StatusSync workers in a real deploy (opt-in, off by default). IdM is reachable again (see §2 + auto-memory `idm-requires-nec-network`).
...
- ✅ **Notifications FYI layer verified END-TO-END LIVE** (both halves). The OutboxDispatcher + StatusSync workers are **opt-in (OFF by default** in committed config — `appsettings.Development.json` has no `Outbox`/`StatusSync` sections). Enabled them for the test via env vars on the `dotnet run` (`Outbox__DispatcherEnabled=true`, `Outbox__IntervalSeconds=3`, `StatusSync__Enabled=true`, `StatusSync__IntervalMinutes=1`) — no committed change. Results:
  - **Accept-driven:** accepting the offer → dispatcher (3s) materialized `Establishment/sent_offer` "Accepted your offer." (Rotana) **and `User/received_offer` "Contract is Active" (the applicant — `offer.is_active`, the new code)**. `opportunity.fulfilled` fired but correctly produced no row (the only applicant is the hired one → excluded). The applicant's bell unread-count showed 1.
  - **StatusSync-driven:** set the event to Upcoming+past-start → the 1-min sweep flipped it Upcoming→Active and emitted `event.started` → dispatcher materialized `Establishment/event` **"Your event has started today"** (Rotana).
  - Older backlog events (`offer.rejected` etc.) also processed correctly. All fixtures restored afterward (offer Pending, notifications cleared). Workers remain off by default; the running API instance has them on via the runtime override (a plain restart reverts to off).
- ✅ **Unevaluated contracts page "حدث خطأ ما!" — root-caused AND fixed** (`matloob-frontend` `dev`, commit `30dabe4`). NOT a token issue (earlier misdiagnosis): the individual `contracts/unevaluated` page reuses the Organizer `ListOffersToEvaluate`, whose `useUnEvaluatedOffers` hook always called **`establishments/offers/unevaluated`** → 400 `establishment_context_required` for an individual in ≥2 establishments — the SAME class as the evaluation-modal bug. Fixed: the hook routes individuals to **`users/offers/unevaluated`** by `userType`. Verified live: 400 → 200, page renders the empty state. Same commit also:
  - **TokenSync** (separate latent fix): the app captured the bearer once at the `/`-route login bridge and never refreshed it on `automaticSilentRenew`, so `attachToken` would send a stale snapshot (~60 min → 401). Added `storage.updateToken` + a `TokenSync` component (mounted in the OIDC provider) that re-syncs storage whenever oidc renews the token.
  - **Refresh tokens — can't enable from frontend:** the IdM *server* supports them (`offline_access` in `scopes_supported`, `refresh_token` in `grant_types_supported`), but the **`matloob:front-dev` client does NOT allow `offline_access`** — verified live (adding it makes the IdM return an error page / login breaks). Renewal stays on the silent iframe; the scope is now env-overridable (`NEXT_PUBLIC_OAUTH_SCOPE`) so refresh tokens flip on without a code change once the IdM admin sets `AllowOfflineAccess=true` on the client.
