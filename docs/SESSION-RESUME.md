...
- **Frontend** (`matloob-frontend`, remote = github.com/EventsCenter/matloob-frontend): `dev`, HEAD `f719ca5` — pushed to `origin/dev` (in sync). **No frontend code changes this session** — the frontend is the contract, every fix was API-side.
...
- ✅ **Evaluation read path + form** verified; the form builds the correct payload but the WRITE endpoint is a frontend bug. Set the offer to WaitingForEvaluation (opportunity Finished); the contract detail then showed the **نموذج التقييم** form (satisfaction rating, "work again?" yes/no, comment, Matloob-platform rating, confirm). Filled it (rating 5, recommend, comment, matloob 5) and submitted — the payload was correct (`rating:5, matloob_evaluation:5, evaluable_id=Rotana, comment`), but it **POSTs to `/api/establishments/evaluations` → 400 `establishment_context_required`** ("caller is a member of multiple establishments"). 🐞 **FRONTEND BUG (not API, not this session's code):** an *individual* applicant's evaluation form hits the *establishment* evaluation endpoint instead of **`/api/users/evaluations`**, so it breaks for any evaluator who is also a member of ≥2 establishments (the test account). The user-side endpoint (`CreateUserEvaluationEndpoint`) exists and is the right target. Needs a frontend fix (endpoint selection by individual-vs-establishment context) — flagged, not changed (no-refactor rule). The evaluation READ is fine (event-hydrated 200).
