...
8. ➡️ **NEXT**: the deferred follow-ups above (Events uploads / nested opportunities; emit the missing notification events), or other frontend gaps. Run `git log --oneline` on branch `feature/api-migration-services-products` for the full slice history.
