...
**Branch:** `feature/api-migration-services-products` (NOT merged to main). ~45 commits, each a clean phase. **Full test suite: 467/467 green.** Run `git log --oneline` for the slice history. Dev Postgres was offline this session — migrations auto-apply on next API start (`Database:AutoMigrate=true` in Dev).
...
1. **Standalone opportunity create — `POST establishments/me/opportunities` multipart criteria + uploads.** The endpoint is JSON-only (`CreateOpportunityEndpoint`) and ignores `success_criteria` + inline uploads; the frontend (`dashboard/opportunities/new`) posts **multipart** (precognition + files) with nested `success_criteria[i][…]` + `uploads[]`. Last piece of full opportunity-criteria parity (the event-wizard path is done). Reuse `EventOpportunitiesSupport`/`SuccessCriteriaSupport` + `OpportunityBuildSupport`; add a multipart branch (or sibling endpoint).
2. **Notification events + scheduler (user chose "build scheduler too").** Legacy notifications `OfferIsActive`, `OpportunityExpired`, `OpportunityFulfilled`, `EventStarted`, `EventEnded` don't fire. Two parts: (a) **handler mappings** for already-emitted transitions — `OpportunityEventTypes.Ended` (manual end → Fulfilled), offer activation → DB notifications in `NotificationOutboxHandler` (cheap); (b) **a new scheduler `BackgroundService`** (sibling to `OutboxDispatcherBackgroundService`) that scans for time-based transitions — opportunity `end_date` passed → Expired + status, event `start_date`/`end_date` → Started/Ended, accepted offer reaching its start → OfferIsActive. **Needs idempotency** (a "notified" marker / one-shot status transition so a notification doesn't re-fire every scan tick) — design carefully; likely a small migration. No `EventEventTypes` exists yet (`event.started`/`event.ended` would be new).
