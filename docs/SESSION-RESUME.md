...
- **Conclusion:** the loop is a deeper `setData`↔`get` round-trip oscillation in the precognition integration for nested-array TextArea paths, not a one-liner. A real fix needs reworking how the controlled TextArea derives/writes its value (or the precognition data-path handling) — i.e. actual frontend refactoring, which the hard no-refactor constraint defers to the frontend owner. Documented; not attempted further.
